...
def test_build_tool_schemas_covers_all_four_week2_tools():
    schemas = build_tool_schemas()
    names = {s["function"]["name"] for s in schemas}
    assert names == {"get_state", "list_legal_actions", "take_action", "flag_anomaly"}
    for s in schemas:
...
def test_tool_models_validate_minimal_args():
    args = TOOL_MODELS["take_action"].model_validate(
        {"actor_id": "p1", "ability_name": "Basic Attack", "reasoning": "why not"}
    )
