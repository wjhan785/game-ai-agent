...
from engine.models import BattleState, Character, Element
...
def build_s1_bulwark(seed: int, defects: DefectFlags) -> tuple[list[Character], list[str]]:
    ...
    party = [
        make_character("p1", "Warden", "party", Element.NEUTRAL, 110, 100, 22, [AEGIS_WARD]),
        make_character("p2", "Blade", "party", Element.FIRE, 90, 90, 20, [EMBER_SLASH, CINDER_BURN]),
        make_character("p3", "Bolt", "party", Element.LIGHTNING, 85, 95, 20, [VOLT_LANCE, SHOCK_COIL]),
    ]
    enemy = [
        make_character("e1", "Cinderfang", "enemy", Element.FIRE, 95, 90, 20, [FLAME_LASH, CINDER_BURN]),
        make_character("e2", "Ashclaw", "enemy", Element.FIRE, 90, 85, 20, [FLAME_LASH]),
        make_character("e3", "Grunt", "enemy", Element.NEUTRAL, 100, 60, 15, []),
    ]
...
def build_s2_pyroclasm(seed: int, defects: DefectFlags) -> tuple[list[Character], list[str]]:
    ...
    party = [
        make_character("p1", "Pyra", "party", Element.FIRE, 90, 90, 20, [CINDER_BURN, EMBER_SLASH]),
        make_character("p2", "Ignis", "party", Element.FIRE, 88, 90, 20, [FLAME_LASH, EMBER_SLASH]),
        make_character("p3", "Warden", "party", Element.NEUTRAL, 105, 100, 20, [AEGIS_WARD]),
    ]
    enemy = [
        make_character("e1", "Colossus", "enemy", Element.NEUTRAL, 160, 70, 15, [CATACLYSM]),
        make_character("e2", "Skirmisher1", "enemy", Element.LIGHTNING, 80, 85, 20, [VOLT_LANCE]),
        make_character("e3", "Skirmisher2", "enemy", Element.ICE, 80, 85, 20, [GLACIAL_SPIKE]),
    ]
    rng = random.Random(seed)
    _perturb(rng, party + enemy)
    return party + enemy, _interleave(party, enemy)
...
def build_s3_deep_freeze(seed: int, defects: DefectFlags) -> tuple[list[Character], list[str]]:
    ...
    party = [
        make_character("p1", "Caster1", "party", Element.LIGHTNING, 85, 90, 18, [CATACLYSM, VOLT_LANCE]),
        make_character("p2", "Caster2", "party", Element.FIRE, 85, 95, 18, [CINDER_BURN, EMBER_SLASH]),
        make_character("p3", "Support", "party", Element.NEUTRAL, 100, 100, 20, [AEGIS_WARD]),
    ]
    enemy = [
        make_character("e1", "Frostweaver1", "enemy", Element.ICE, 85, 90, 18, [FROST_BIND, GLACIAL_SPIKE]),
        make_character("e2", "Frostweaver2", "enemy", Element.ICE, 85, 90, 18, [FROST_BIND, GLACIAL_SPIKE]),
        make_character("e3", "IceboundGrunt", "enemy", Element.NEUTRAL, 110, 60, 15, []),
    ]
    rng = random.Random(seed)
    _perturb(rng, party + enemy)
    return party + enemy, _interleave(party, enemy)
...
def build_s4_attrition(seed: int, defects: DefectFlags) -> tuple[list[Character], list[str]]:
    ...
    both sides."""
    party = [
        make_character("p1", "Venomblade", "party", Element.NEUTRAL, 95, 90, 18, [TOXIC_STRIKE, EMBER_SLASH]),
        make_character("p2", "Breaker", "party", Element.LIGHTNING, 90, 85, 18, [CRIPPLING_BLOW, VOLT_LANCE]),
        make_character("p3", "Medic", "party", Element.NEUTRAL, 95, 90, 18, [MENDING_LIGHT]),
    ]
    enemy = [
        make_character("e1", "Venomclaw", "enemy", Element.NEUTRAL, 95, 90, 18, [TOXIC_STRIKE, GLACIAL_SPIKE]),
        make_character("e2", "Crusher", "enemy", Element.LIGHTNING, 90, 85, 18, [CRIPPLING_BLOW, CATACLYSM]),
        make_character("e3", "Healer", "enemy", Element.NEUTRAL, 95, 90, 18, [MENDING_LIGHT]),
    ]
    rng = random.Random(seed)
    _perturb(rng, party + enemy)
    return party + enemy, _interleave(party, enemy)
...
def build_s5_lockdown(seed: int, defects: DefectFlags) -> tuple[list[Character], list[str]]:
    ...
    party = [
        make_character("p1", "Quickblade", "party", Element.FIRE, 90, 90, 22, [EMBER_SLASH, VOLT_LANCE]),
        make_character("p2", "Duelist", "party", Element.ICE, 88, 90, 22, [GLACIAL_SPIKE]),
        make_character("p3", "Support", "party", Element.NEUTRAL, 100, 100, 20, [AEGIS_WARD]),
    ]
    enemy = [
        make_character("e1", "Shockmaw", "enemy", Element.LIGHTNING, 90, 90, 20, [SHOCK_COIL, VOLT_LANCE]),
        make_character("e2", "Stormcaller", "enemy", Element.LIGHTNING, 85, 90, 20, [SHOCK_COIL]),
        make_character(
            "e3",
            "Overtuned",
            "enemy",
...
def build_s6_last_stand(seed: int, defects: DefectFlags) -> tuple[list[Character], list[str]]:
    ...
    party = [
        make_character("p1", "Cleric", "party", Element.NEUTRAL, 95, 110, 22, [MENDING_LIGHT, REVIVE]),
        make_character("p2", "Fighter1", "party", Element.FIRE, 75, 85, 18, [EMBER_SLASH]),
        make_character("p3", "Fighter2", "party", Element.LIGHTNING, 80, 85, 18, [VOLT_LANCE]),
    ]
    enemy = [
        make_character("e1", "Behemoth", "enemy", Element.LIGHTNING, 130, 90, 18, [CATACLYSM]),
        make_character("e2", "Adds1", "enemy", Element.NEUTRAL, 55, 60, 15, []),
        make_character("e3", "Adds2", "enemy", Element.NEUTRAL, 55, 60, 15, []),
    ]
    rng = random.Random(seed)
    _perturb(rng, party + enemy)
    return party + enemy, _interleave(party, enemy)
